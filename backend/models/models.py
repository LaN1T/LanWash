    ...
    def __table_args__(cls):
        return (
            ...
            Index(
                ...
                "userId",
                unique=True,
                sqlite_where=(cls.isPrimary.is_(True)),
                postgresql_where=(cls.isPrimary.is_(True)),
            ),
        )
...
class WashTypeIncludedExtra(Base):
    ...
    washTypeId = Column(
        String, ForeignKey("wash_types.id", ondelete="CASCADE"), primary_key=True
    )
    extraServiceId = Column(
...
class WashTypeConsumable(Base):
    ...
    consumableId = Column(
        String, ForeignKey("consumables.id", ondelete="CASCADE"), primary_key=True
    )
...
class Service(Base):
    ...
    __table_args__ = (Index("ix_services_category", "category"),)
    id = Column(String, primary_key=True)
    name = Column(String, nullable=False)
    description = Column(String, nullable=False, default="")
    price = Column(Integer, nullable=False, default=0)
...
class Promo(Base):
    ...
    __table_args__ = (Index("ix_promos_wash_type", "washTypeId"),)
    id = Column(String, primary_key=True)
    washTypeId = Column(String, ForeignKey("wash_types.id"), nullable=False)
    name = Column(String, nullable=False)
    description = Column(String, nullable=False, default="")
    ...
    )  # 0 = использовать basePrice типа мойки со скидкой
    discountPercent = Column(Integer, nullable=False, default=0)
...
class PromoIncludedExtra(Base):
    ...
    promoId = Column(
        String, ForeignKey("promos.id", ondelete="CASCADE"), primary_key=True
    )
    extraServiceId = Column(
        String, ForeignKey("services.id", ondelete="CASCADE"), primary_key=True
    )
...
class WasherNote(Base):
    ...
    __table_args__ = (Index("ix_washer_notes_username", "username"),)
    id = Column(Integer, primary_key=True, autoincrement=True)
    username = Column(String, nullable=False)
...
class ShiftTemplate(Base):
    ...
    __table_args__ = (Index("ix_shift_templates_owner", "ownerUsername"),)
    id = Column(Integer, primary_key=True, autoincrement=True)
...
class Referral(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "referrerId", "referredId", name="uq_referral_referrer_referred"
        ),
...
class Tip(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "appointmentId", "washerUsername", name="uq_tip_appointment_washer"
        ),
...
class SupportMessage(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    chatId = Column(
        Integer, ForeignKey("support_chats.id", ondelete="CASCADE"), nullable=False
    )
