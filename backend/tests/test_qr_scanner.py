...
class TestQrScanner:
    ...
    async def _create_appointment(
        self, async_client, token, appt_id, date_time, status="scheduled", owner="client_test", assigned_washer=None
    ):
        ...
        payload = {
            ...
            "isSeenByClient": True,
            "originalPrice": 1000,
            "assignedWasher": json.dumps([assigned_washer]) if assigned_washer else "[]",
            "promoId": None,
            "box_index": 0,
        }
    ...
    async def test_qr_endpoint_owner(self, async_client, client_token):
        await self._create_appointment(async_client, client_token, "qr_appt_1", "2099-05-01T10:00:00")
    ...
    async def test_qr_endpoint_admin(self, async_client, admin_token, client_token):
        await self._create_appointment(async_client, client_token, "qr_appt_2", "2099-05-02T10:00:00")
        resp = await async_client.get(
    ...
    @pytest.mark.asyncio
    async def test_qr_endpoint_assigned_washer(self, async_client, admin_token, washer_token):
        await self._create_appointment(
            async_client, admin_token, "qr_appt_3", "2099-05-03T10:00:00", assigned_washer="washer_test"
        )
        resp = await async_client.get(
    ...
    async def test_qr_endpoint_forbidden_unauthorized(self, async_client, client_token):
        ...
        await self._create_appointment(async_client, client_token, "qr_appt_4", "2099-05-04T10:00:00")
    ...
    @pytest.mark.asyncio
    async def test_scan_qr_updates_status(self, async_client, admin_token, washer_token, client_token):
        # Только админ может назначать мойщика при создании
        await self._create_appointment(
            async_client, admin_token, "qr_appt_5", "2099-05-05T10:00:00",
    ...
    @pytest.mark.asyncio
    async def test_scan_qr_wrong_status(self, async_client, admin_token, washer_token, client_token):
        await self._create_appointment(
            async_client, admin_token, "qr_appt_7", "2099-05-07T10:00:00",
