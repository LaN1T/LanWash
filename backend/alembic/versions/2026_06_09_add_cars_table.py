...
def upgrade() -> None:
    ...
    # Migrate existing data: for each user with non-empty carModel/carNumber,
    # create a Car row.
    # Use a safe approach: split carModel on first space into brand/model,
    # or use whole string as brand.
    conn = op.get_bind()
    if conn.dialect.name == "sqlite":
        ...
        conn.execute(
            ...
            SELECT
                id,
                CASE
                    WHEN instr(carModel, ' ') > 0
                        THEN substr(carModel, 1, instr(carModel, ' ') - 1)
                    ELSE carModel
                END,
                CASE
                    WHEN instr(carModel, ' ') > 0
                        THEN substr(carModel, instr(carModel, ' ') + 1)
                    ELSE ''
                END,
                carNumber,
                1
        ...
        conn.execute(
            ...
            SELECT
                ...
                END,
                CASE
                    WHEN POSITION(' ' IN carModel) > 0
                        THEN SUBSTRING(carModel FROM POSITION(' ' IN carModel) + 1)
                    ELSE ''
                END,
                carNumber,
                TRUE
