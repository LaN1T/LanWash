...
async def get_by_user(
    ...
    if current_user.username != username.lower() and current_user.role != "admin":
        raise HTTPException(
            status.HTTP_403_FORBIDDEN, "У вас нет доступа к чужим заметкам"
        )

    svc = NotesService(db)
    return await svc.get_by_user(
        username.lower(), pagination.per_page, pagination.offset
    )
...
async def create(
    ...
    req: NoteRequest,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(check_roles(["washer", "admin"])),
):
    """Создать заметку (мойщик)."""
    if current_user.username != username.lower() and current_user.role != "admin":
        raise HTTPException(
            status.HTTP_403_FORBIDDEN,
            "Вы не можете создавать заметки от имени другого пользователя",
        )
