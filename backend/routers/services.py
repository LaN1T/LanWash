...
@limiter.limit("60/minute")
async def get_service_favorites(
    request: Request,
    username: str,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
    ...
    if current_user.username != username.lower() and current_user.role != "admin":
        raise HTTPException(
            status.HTTP_403_FORBIDDEN, "У вас нет доступа к чужим избранным услугам"
        )
    svc = ServicesService(db)
...
async def toggle_service_favorite(
    ...
    if current_user.username != req.username.lower() and current_user.role != "admin":
        raise HTTPException(
            status.HTTP_403_FORBIDDEN, "У вас нет прав на изменение чужого избранного"
        )
    svc = ServicesService(db)
...
@limiter.limit("10/minute")
async def toggle_extra_favorite(
    request: Request,
    req: ToggleExtraFavoriteRequest,
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    if current_user.username != req.username.lower() and current_user.role != "admin":
