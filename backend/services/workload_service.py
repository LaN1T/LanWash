...
class WorkloadService:
    ...
    async def get_appointment_duration(
        ...
        res_wt = await db.execute(
            select(WashType.durationMinutes).where(WashType.id == wash_type_id)
        )
        base_duration = res_wt.scalar() or 30

        # 2. Получаем услуги, уже включённые в этот тип мойки
        res_included = await db.execute(
        ...
        if promo_id:
            res_promo = await db.execute(
                select(Promo.duration).where(Promo.id == promo_id)
            )
            ...
            res_promo_inc = await db.execute(
        ...
        if extra_ids:
            ...
            if filtered_ids:
                res_extras = await db.execute(
                    select(Service.durationMinutes).where(Service.id.in_(filtered_ids))
                )
    ...
    async def get_appointment_durations_batch(
        ...
        if promo_ids:
            ...
            pri_res = await db.execute(
                select(
                    PromoIncludedExtra.promoId, PromoIncludedExtra.extraServiceId
        ...
        if all_service_ids:
            svc_res = await db.execute(
                select(Service.id, Service.durationMinutes).where(
                    Service.id.in_(all_service_ids)
                )
            )
