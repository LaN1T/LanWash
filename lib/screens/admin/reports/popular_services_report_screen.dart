...
class _PopularServicesReportScreenState
  ...
  Future<void> _fetchReport() async {
    ...
    try {
      final report = await sl<ApiService>().getPopularAdditionalServices(
        _periodParam,
  ...
  Future<void> _exportPdf() async {
    ...
    final data =
        _report!.data.map((e) => [e.serviceName, e.count.toString()]).toList();

    final fileName = 'Популярные услуги_${_periodParam}_$_selectedCategory';
    final pdfBytes = await PdfExportService.createPdfBytes(
      'Отчёт: Популярные услуги за $_periodLabel ($_selectedCategory)',
      headers,
      data,
    );
    ...
    if (kIsWeb) {
      ...
      await PdfExportService.showExportDialog(
        context,
        title: 'Отчёт: Популярные услуги за $_periodLabel ($_selectedCategory)',
        fileName: fileName,
        pdfBytes: pdfBytes,
      );
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Популярные услуги',
                        ...
                        ReportSummaryCard(
                          children: [
                            ...
                            ReportSummaryItem(
                              ...
                              value:
                                  '${_report!.data.fold<int>(0, (sum, e) => sum + e.count)}',
                            ),
                          ],
                        ),
                        const SizedBox(height: 16),
