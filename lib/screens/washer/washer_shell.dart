...
class _WasherShellState extends State<WasherShell> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Row(children: [
          Container(width: 34, height: 34, decoration: const BoxDecoration(shape: BoxShape.circle, gradient: AppStyles.primaryGradient), child: const Icon(Icons.local_car_wash, color: Colors.white, size: 18)),
          const SizedBox(width: 10),
          Text(_tabIndex == 0 ? 'Мои записи' : 'Мои заметки', style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w700, color: AppStyles.textPrimary)),
        ]),
      ),
      ...
      bottomNavigationBar: NavigationBar(
        ...
        destinations: const [
          NavigationDestination(icon: Icon(Icons.calendar_today), label: 'Записи'),
  ...
  Widget _buildAppointmentsTab() {
    ...
    final filteredAppts = appts.where((a) => a.dateTime.year == _selectedDay.year && a.dateTime.month == _selectedDay.month && a.dateTime.day == _selectedDay.day).toList();
                        ...
                        child: Padding(
                          ...
                          child: Container(
                            decoration: BoxDecoration(
                              ...
                              border: Border.all(color: isSelected ? AppStyles.primary : (isToday ? AppStyles.primary : Colors.grey.shade200), width: isToday ? 2 : 1),
                            ),
                            child: Column(
                              mainAxisAlignment: MainAxisAlignment.center,
                              children: [
                                Text(DateFormat('E', 'ru').format(d).toUpperCase(), style: TextStyle(color: isSelected ? Colors.white : AppStyles.textSecondary, fontSize: 9)),
                                Text('${d.day}', style: TextStyle(color: isSelected ? Colors.white : AppStyles.textPrimary, fontSize: 14, fontWeight: FontWeight.bold)),
                                if (count > 0)
                                  Container(
                                    width: 14, height: 14, alignment: Alignment.center,
                                    decoration: BoxDecoration(color: isSelected ? Colors.white : AppStyles.primary, shape: BoxShape.circle),
                                    child: Text('$count', style: TextStyle(color: isSelected ? AppStyles.primary : Colors.white, fontSize: 8, fontWeight: FontWeight.bold)),
                                  )
                              ],
                            ),
                          ),
          ...
          child: Column(
            ...
            children: [
              Row(children: [
                Container(
                  ...
                  child: Row(children: [
                    ...
                    const SizedBox(width: 4),
                    Text(AppStyles.statusLabel(a.status), style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: statusColor)),
                  ]),
                ),
                const Spacer(),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                  decoration: BoxDecoration(color: AppStyles.primaryBg, borderRadius: BorderRadius.circular(6)),
                  child: Text(timeStr, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppStyles.primary)),
                ),
              ]),
              ...
              Row(children: [
                ...
                Expanded(child: Text(a.clientName, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w500))),
              ]),
              const SizedBox(height: 6),
              Row(children: [
                const Icon(Icons.directions_car, size: 16, color: AppStyles.textSecondary),
                const SizedBox(width: 6),
                Text(a.carModel, style: const TextStyle(fontSize: 13, color: AppStyles.textSecondary)),
                const SizedBox(width: 8),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2),
                  decoration: BoxDecoration(color: AppStyles.primaryBg, borderRadius: BorderRadius.circular(4)),
                  child: Text('Бокс №${a.box_index + 1}', style: const TextStyle(color: AppStyles.primary, fontSize: 12, fontWeight: FontWeight.w600)),
                ),
              ]),
            ],
          ),
  ...
  showModalBottomSheet(
      ...
      builder: (_) => Container(
          ...
          child: Column(
            ...
            children: [
              Text('Запись: ${a.clientName}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 20),
              const Text('Статус'),
              const SizedBox(height: 10),
              Wrap(
                spacing: 8,
                children: ['scheduled', 'in_progress', 'completed'].map((s) => ElevatedButton(
                  onPressed: () async {
                    Navigator.pop(context);
                    await provider.updateAppointment(a.copyWith(status: s), context.read<AuthProvider>());
                  },
